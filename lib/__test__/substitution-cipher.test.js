  ...
  describe('randomUniqueNumbers(length)', () => describe(
    ...
    () => RangeIterable.range(5).map(x => 1 << x).forEach(length => {
      ...
      describe(`length = ${length}, array = [${array.join(', ')}]`, () => {
        array.forEach(x => describe(`x = ${x}`, () => {
          test('should be a number', () => expect(x).toBeType('number'))
          test('should not be negative', () => expect(x).toBeGreaterThanOrEqual(0))
          test('should be less than length', () => expect(x).toBeLessThan(length))
          test('should be an integer', () => expect(x % 1).toBe(0))
        }))
      })
    })
  ))
